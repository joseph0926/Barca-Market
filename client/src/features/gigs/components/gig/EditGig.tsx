...
import { useGigSchema } from '@/features/gigs/hooks/useGigSchema';
import { GIG_MAX_LENGTH, IAllowedGigItem, ICreateGig, ISellerGig, IShowGigModal } from '../../interfaces/gig.interface';
import { gigInfoSchema } from '@/features/gigs/schemes/gig.schema';
import { useUpdateGigMutation } from '@/features/gigs/services/gigs.service';
import TagsInput from '@/features/gigs/components/gig/components/TagsInput';
          ...
          <div className="border-grey left-0 top-0 z-10 mt-4 block rounded border bg-white p-6">
            ...
            <div className="xs:grid-cols-1 grid md:grid-cols-5">
              ...
              <div className="col-span-4 flex gap-x-4 md:w-11/12 lg:w-8/12">
                <Button
                  disabled={isLoading}
                  className="rounded bg-orange-500 px-8 py-3 text-center text-sm font-bold text-white hover:bg-orange-400 focus:outline-none md:py-3 md:text-base"
                  label="Edit Gig"
                  onClick={onEditGig}
                />
                <Button
