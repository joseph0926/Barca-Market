...
import { isFetchBaseQueryError, lowerCase, showErrorToast } from '@/shared/utils/utils.service';
...
import { chatListMessageReceived, chatListMessageUpdated } from '@/features/chat/services/chat.utils';
                ...
                {!data.isRead ? (
                  <>
                    {data.receiverUsername === authUser.username ? (
                      <FaCircle className="mt-2 text-orange-500" size={8} />
                    ) : (
                      <FaCheck className="mt-2" size={8} />
                    )}
                  </>
                ) : (
                  <FaCheckDouble className="mt-2 text-orange-500" size={8} />
                )}
              </div>
            </div>
          </div>
