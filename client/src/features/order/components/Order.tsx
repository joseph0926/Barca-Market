...
import { useGetOrderByOrderIdQuery } from '@/features/order/services/order.service';
import DeliveryTimer from '@/features/order/components/DeliveryTimer';
import OrderActivities from '@/features/order/components/order-activities/OrderActivities';
import OrderDetailsTable from '@/features/order/components/OrderDetailsTable';
            ...
            <div className="mt-4 flex flex-col justify-between bg-white md:flex-row">
              ...
              <div className="mb-4 ml-5 w-full justify-center self-center text-left md:mr-3 md:w-1/3 md:text-right">
                {order && order.delivered && order.buyerUsername === authUser.username && (
                  <Button
                    className="rounded bg-orange-500 px-2 py-2 text-center text-sm font-bold text-white hover:bg-orange-400 focus:outline-none md:px-4 md:py-2 md:text-base"
                    label="View Delivery"
                    onClick={() => {
