...
import { IOffer, IOrderDocument, IOrderInvoice } from '@/features/order/interfaces/order.interface';
import { useCreateOrderMutation } from '@/features/order/services/order.service';
import Invoice from '@/features/order/components/Invoice/Invoice';
  ...
  const startOrder = async (): Promise<void> => {
    try {
      ...
      const order: IOrderDocument = {
        ...
        sellerImage: `${gigRef?.current?.profileImage}`,
        ...
        <div className="order-last w-full p-4 lg:order-first lg:w-2/3">
          <div className="mb-4 flex w-full flex-col flex-wrap bg-[#d4edda] p-4">
            ...
            <div className="flex gap-1">
              ...
              >
                <div className="cursor-pointer text-red-400 underline">download your invoice</div>
              </PDFDownloadLink>
            </div>
          </div>
          <div className="border-grey border">
            ...
            <div className="flex flex-col px-4 pb-4">
              ...
              />
              <Button
                className="mt-3 rounded bg-orange-500 px-6 py-3 text-center text-sm font-bold text-white hover:bg-orange-400 focus:outline-none md:px-4 md:py-2 md:text-base"
                label="Start Order"
                onClick={startOrder}
              />
            </div>
